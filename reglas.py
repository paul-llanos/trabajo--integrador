...
def verificaion_de_credenciales(
    axuliar: bool,
    nombre_admin_res: str,
    nombre_servidor: str,
    ...
    if (
        nombre_admin_res != NOMBRE_ADMINISTRADOR
        or nombre_servidor != NOMBRE_SERVIDOR
        or sistema_operativo != SISTEMA_OPERATIVO
        or tipo_servidor != TIPO_SERVIDOR
    ):
        print(
            f"\n=============================="
...
def verificacion_de_cpu(cpu_usada: int) -> list:
    ...
    if cpu_usada > CPU_MAX:
        ...
        resultado_cpu[3] = (
            resultado_cpu[3] + 1
        )  # sumamamos el contador de riesgo en la posicion 3
...
def verificaciion_de_cuello_de_botella(
    cpu_usada: int, ram_usada: int, diagnostico_cpu: list, diagnostico_ram: list
) -> None:
    """
    ...
    if cuello_de_botella > RAM_CPU_CUELLO_DE_BOTELLA:
        if cpu_usada < ram_usada:
            ...
            diagnostico_ram[1] = (
                diagnostico_ram[1]
                + f" | RAM: {ram_usada}% cuello de botella respecto a CPU: {cpu_usada}% Diferencia de: {cuello_de_botella}%)"
            )
            ...
            diagnostico_cpu[1] = (
                diagnostico_cpu[1]
                + f" |  CPU: {cpu_usada}% tiene un cuello de botella respecto a la RAM: {ram_usada}% La diferencia es de: {cuello_de_botella}% de uso)"
            )
...
def verificacion_de_intensidad_de_cpu_por_usario(
    ...
    """
    intensidad_cpu_usuarios = calculo_intensidad_cpu_usuarios(
        cpu_usada, usuarios_conectados
    )
    ...
    if intensidad_cpu_usuarios > CANTIDAD_MAXIMA_DE_CPU_POR_USUARIO:
        ...
        diagnostico_usuarios[1] = (
            diagnostico_usuarios[1]
            + f" | Carga anormal. Cada usuario consume {intensidad_cpu_usuarios}% de CPU. Sospecha de ataque cibernetico."
        )
        diagnostico_usuarios[2] = (
            diagnostico_usuarios[2] + f" | investigue a ese usuario."
        )
        diagnostico_usuarios[3] = diagnostico_usuarios[3] + 1


def verificacion_de_estado_critico_en_ram_y_disco(
    ram_usada: int, espacio_disco: float, diagnostico_ram: list, diagnostico_disco: list
) -> None:
    """
    ...
    if diagnostico_ram[0] == "critico" and diagnostico_disco[0] == "critico":
        ...
        diagnostico_ram[1] = (
            diagnostico_ram[1]
            + f" | cuidado tiene la ram {ram_usada}% y a su disco le quedan menos de {espacio_disco} GB su servidor puede caer"
        )
        diagnostico_disco[1] = (
            diagnostico_disco[1]
            + f" | cuidado tiene el disco {espacio_disco} GB y a su ram {ram_usada}% su servidor puede colapsar"
        )

        diagnostico_ram[2] = (
            diagnostico_ram[2] + f" | Limite el consumo de ram por usuario."
        )
        diagnostico_disco[2] = (
            diagnostico_disco[2] + f" | elimine archivos temporales del usuario."
        )


def verificacion_de_cantidad_de_procesos_por_usuario(
    usuarios_conectados: int, procesos_activos: int, diagnostico_procesos: list
) -> None:
    """
    ...
    procesos_por_usuario = calculo_de_cantidad_de_procesos_por_usuarios(
        usuarios_conectados, procesos_activos
    )
    if procesos_por_usuario > MAXIMOS_PROCESOS_POR_USUARIO:
        diagnostico_procesos[0] = diagnostico_procesos[0] + f" | CUIDADO"
        diagnostico_procesos[1] = (
            diagnostico_procesos[1]
            + f" | El promedio de PROCESOS de cada usuario es {procesos_por_usuario} . Posible ataque cibernetico o fuga de hilos"
        )
        diagnostico_procesos[2] = (
            diagnostico_procesos[2]
            + f" | Haga una verificacion del servidor para estar seguro."
        )
        diagnostico_procesos[3] = diagnostico_procesos[3] + 1


def verificacion_de_vulnerabilidad(
    tipo_servidor: str, estado_firewall: str, sistema_operativo: str
) -> list:
    """
    ...
    if (tipo_servidor == "base de datos" or tipo_servidor == "archivos") and (
        ...
        diagnostico_seguridad[1] = (
            f"El servidor de tipo {tipo_servidor} presenta una vulnerabilidad crítica al ejecutar {sistema_operativo} con el firewall inactivo."
        )
